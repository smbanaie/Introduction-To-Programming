...
### کاوش REPL پایتون
این تمرین‌ها را در REPL پایتون امتحان کنید:

۱. **عملیات پایه**: عملیات ریاضی ساده را امتحان کنید
   ```python
   >>> 5 + 3
   >>> 10 * 2
   >>> 15 / 3
...
۲. **عملیات رشته**: با متن آزمایش کنید
   ```python
   >>> "Hello" + " " + "World"
   >>> "Python".upper()
   >>> len("Hello")
...
۳. **متغیرها**: متغیرها را ایجاد و استفاده کنید
   ```python
   >>> name = "Alice"
   >>> age = 25
...
### چالش‌های ایجاد اسکریپت

۱. **Hello World**: اسکریپتی ایجاد کنید که "Hello, World!" را چاپ کند
۲. **سلام شخصی**: اسکریپت را تغییر دهید تا نام کاربر را بپرسد و به صورت شخصی سلام کند
۳. **ماشین‌حساب ساده**: اسکریپتی بنویسید که دو عدد را بپرسد و مجموع آنها را چاپ کند

### اشکال‌زدایی خطا
این خطاهای رایج را شناسایی و رفع کنید:

```python
# این خطاها را رفع کنید:
print("Hello)          # گیومه گم شده
Print("Hello")         # حروف اشتباه
print(Hello)           # گیومه برای رشته گم شده
print("Hello"          # پرانتز بسته شدن گم شده
```

### سؤالات بلیط خروج
۱. چگونه REPL پایتون را شروع می‌کنید؟
۲. تفاوت بین اسکریپت و REPL چیست؟
۳. یک دستور پایتون برای چاپ نام خود بنویسید
